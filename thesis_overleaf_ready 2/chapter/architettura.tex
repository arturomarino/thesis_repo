...
\centering
\textbf{Copernicus NetCDF archive}\\[0.7em]
$\downarrow$\\[0.5em]
\textbf{Lazy Xarray/Dask access}\\
variable selection $+$ land--sea mask\\[0.7em]
$\downarrow$\\[0.5em]
\textbf{Chronological split}\\
1994--1997 training $\mid$ 1998 validation $\mid$ 1999 test\\[0.7em]
$\downarrow$\\[0.5em]
\textbf{Depth-dependent standardization}\\
statistics fitted on the training split only\\[0.7em]
$\downarrow$\\[0.5em]
\textbf{Lazy forecast samples}\\
$[X_{t-2},X_{t-1},X_t]\mapsto X_{t+1}$\\[0.7em]
$\downarrow$\\[0.5em]
\textbf{Probabilistic 3D U-Net}\\
predictive mean $\mu$ $+$ log-variance $\log\sigma^2$\\[0.7em]
$\downarrow$\\[0.5em]
\textbf{Masked training and evaluation}\\
Gaussian NLL, RMSE, MAE, coverage, and skill versus persistence
\end{minipage}}
